...
pub static CURRENT_USER: std::sync::OnceLock<RwSignal<Option<UserProfile>>> = std::sync::OnceLock::new();
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        spawn_local(async {
            ...
            if let Some(stored_user) = load_user_from_storage() {
                web_sys::console::log_1(&format!("Loading user from storage: {} {}", stored_user.name, stored_user.surname).into());
        ...
        if let Ok(Some(storage)) = window.local_storage() {
            if let Ok(serialized) = serde_json::to_string(user) {
                match storage.set_item("clock_it_user", &serialized) {
                    ...
                    Err(e) => web_sys::console::log_1(&format!("Error saving to localStorage: {:?}", e).into()),
                }
            }
        }
    }
